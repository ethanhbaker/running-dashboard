...
| `gpx/<activity_id>.gpx` | `download_activity_file(activity_id, format="gpx", output_dir="<repo>/data/raw/gpx")` | One call per outdoor run you want on the Training page's route heatmap. Pull every real (non-treadmill) run in whatever window you want represented - the current data covers ~2 months and ~48 runs. `compute.py`'s `build_route_heatmap()` reads every `.gpx` file in this folder, groups them by the first word of the track name (this account's activity names all start with a location, e.g. "Worcester Running"), and projects each into its own local SVG. No GPX = the Training page shows an honest "not pulled yet" empty state instead of a heatmap. |
